...
class MemoryStore: ObservableObject, Refreshable {
    ...
    /// Apple Silicon keeps memory on-package and exposes no equivalent of the
    /// Intel `MEM_SLOTS_PROXIMITY` sensor, so this stays nil and the UI shows N/A.
    @Published var temp: Double?
    @Published var usageHistory: [Double] = []
    @Published var estimatedMemoryBandwidthGBs: Int?
    @Published var vmPressureLevel: MemoryVMPressureLevel?
    ...
    @objc func refresh() {
        ...
        estimatedMemoryBandwidthGBs = AppleSiliconIOReport.shared.lastBandwidthGBs
        vmPressureLevel = MemoryVMPressure.currentLevel()
        usageHistory = (usageHistory + [usedPercentage]).suffix(LineChart.defaultMaxPointCount)
        Print("MemoryStore BWest", estimatedMemoryBandwidthGBs as Any)
        writeToContainer()
    }
