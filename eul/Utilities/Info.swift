...
enum Info {
    ...
    static func getNetworkUsage(forDevice: String?, _ onData: @escaping (NetworkUsage, [NetworkPort], NetworkPort?, String) -> Void) {
        // TO-DO: use Combine
        shellAsync("networksetup -listnetworkserviceorder") {
            ...
            DispatchQueue.main.async {
                onData(
                    NetworkUsage(inBytes: inBytes ?? 0, outBytes: outBytes ?? 0),
                    services,
                    currentActivePort,
                    device
                )
