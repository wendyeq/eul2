...
enum AutoUpdate {
    ...
    static let appUrl = fileManager.temporaryDirectory.appendingPathComponent("eul2.app")
    static let zipUrl = fileManager.temporaryDirectory.appendingPathComponent("eul2.app.zip")
    ...
    static func downloadLatest(completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: "https://github.com/wendyeq/eul2/releases/latest/download/eul2.app.zip") else {
            completion(false)
            return
        }
