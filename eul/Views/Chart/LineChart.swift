...
struct LineChart: View {
    ...
    var body: some View {
        ZStack {
            Group {
                closedPath()
                    .fill(Color.text)
                path()
                    .stroke(Color.text, style: StrokeStyle(lineWidth: lineWidth, lineJoin: .round))
            }
        ...
        .accessibilityLabel("accessibility.usage_history".localized())
        .accessibilityValue(points.last.map { String(format: "%.0f%%", $0) } ?? "")
    }

    private var lineWidth: CGFloat {
        NSWorkspace.shared.accessibilityDisplayShouldIncreaseContrast ? 1.5 : Self.minimumLineHeight
    }
}
